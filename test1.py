...
def plot_measurements(tracks):
    ...
    for track in tracks:
        for measurement, state in track['measurements']:
            # Adjust the unpacking to match the actual structure of the measurement tuple
            r, az, el, t, *rest = measurement  # Use *rest to capture any additional elements
            times.append(t)
            ranges.append(r)
            azimuths.append(az)
            ...
            # Assuming Sf contains the state vector with positions at indices 0, 1, 2
            Sf_ranges.append(track['Sf'][0])
            Sf_azimuths.append(track['Sf'][1])
            Sf_elevations.append(track['Sf'][2])

    # Plot Range vs Time
    plt.figure(figsize=(10, 6))
    plt.scatter(times, ranges, label='Range', color='blue')
    plt.scatter(times, Sf_ranges, label='Sf Range', color='red', marker='x')
    plt.xlabel('Time')
    plt.ylabel('Range')
    plt.title('Range and Sf Range vs Time')
    plt.legend()
    cursor = mplcursors.cursor(hover=True)
    cursor.connect("add", lambda sel: sel.annotation.set_text(
        f"Track ID: {track_ids[sel.index]}\nRange: {ranges[sel.index]}\nSf Range: {Sf_ranges[sel.index]}\nTime: {times[sel.index]}"
    ))
    plt.show()

    # Plot Azimuth vs Time
    plt.figure(figsize=(10, 6))
    plt.scatter(times, azimuths, label='Azimuth', color='blue')
    plt.scatter(times, Sf_azimuths, label='Sf Azimuth', color='red', marker='x')
    plt.xlabel('Time')
    plt.ylabel('Azimuth')
    plt.title('Azimuth and Sf Azimuth vs Time')
    plt.legend()
    cursor = mplcursors.cursor(hover=True)
    cursor.connect("add", lambda sel: sel.annotation.set_text(
        f"Track ID: {track_ids[sel.index]}\nAzimuth: {azimuths[sel.index]}\nSf Azimuth: {Sf_azimuths[sel.index]}\nTime: {times[sel.index]}"
    ))
    plt.show()

    # Plot Elevation vs Time
    plt.figure(figsize=(10, 6))
    plt.scatter(times, elevations, label='Elevation', color='blue')
    plt.scatter(times, Sf_elevations, label='Sf Elevation', color='red', marker='x')
    plt.xlabel('Time')
    plt.ylabel('Elevation')
    plt.title('Elevation and Sf Elevation vs Time')
    plt.legend()
    cursor = mplcursors.cursor(hover=True)
    cursor.connect("add", lambda sel: sel.annotation.set_text(
        f"Track ID: {track_ids[sel.index]}\nElevation: {elevations[sel.index]}\nSf Elevation: {Sf_elevations[sel.index]}\nTime: {times[sel.index]}"
    ))
    plt.show()
...
def main():
        ...
        if len(group) == 1:  # Single measurement
            ...
            if not assigned:
                ...
                tracks.append({
                    ...
                    'current_state': 'Poss1',
                    'Sf': kalman_filter.Sf.copy()  # Store the state vector
                })
            ...
            for track_id, best_report in best_reports:
                ...
                hit_counts[track_id] = hit_counts.get(track_id, 0) + 1
                tracks[track_id]['Sf'] = kalman_filter.Sf.copy()  # Update the state vector
    ...
    # Plot the measurements and state vectors
    plot_measurements(tracks)
